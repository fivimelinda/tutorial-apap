...
public class RestoranRestController {
	...
	private RestoranModel createRestoran(@Valid @RequestBody RestoranModel restoran, BindingResult bindingResult) {
		if(bindingResult.hasFieldErrors()) {
			...
			return
					restoranRestService.createRestoran(restoran);
		}
	}
	...
	private List<RestoranModel> retrieveListRestoran(){
		return
				restoranRestService.retrieveListRestoran();
	}
	
	@GetMapping(value="/restoran/{idRestoran}/status")
	private Mono<String> getStatus(@PathVariable Long idRestoran) {return restoranRestService.getStatus(idRestoran);}
